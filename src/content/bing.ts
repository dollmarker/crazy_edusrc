...
const logger = new Logger('Bing内容脚本');
...
if (!window.hackingSidebarInjected) {
  ...
  // 创建侧边栏管理器
  const sidebarManager = SidebarFactory.create('bing');
  ...
  if (sidebarManager) {
    ...
    const initialize = async () => {
      ...
      // 等待关键DOM元素出现（右侧栏或主要内容区域）
      const mainContainer = await waitForElement(
        `${BING_SELECTORS.RIGHT_RAIL}, ${BING_SELECTORS.MAIN_CONTAINER}`,
        5000
      );
