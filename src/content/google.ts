...
const logger = new Logger('Google内容脚本');
...
if (!window.hackingSidebarInjected) {
  ...
  // 创建侧边栏管理器
  const sidebarManager = SidebarFactory.create('google');
  ...
  if (sidebarManager) {
    ...
    const initialize = async () => {
      ...
      const mainContainer = await waitForElement(
        `${GOOGLE_SELECTORS.RIGHT_COLUMN}, ${GOOGLE_SELECTORS.CENTER_COL}`,
        5000
      );
      
      if (mainContainer) {
        // 在浏览器空闲时初始化侧边栏，避免阻塞页面渲染
        runWhenIdle(() => {
          sidebarManager.init(true);
        }, 1000);
      } else {
        logger.warn('关键DOM元素未找到，延迟初始化');
        // 兜底方案：如果5秒内未找到，仍然尝试初始化
        runWhenIdle(() => {
          sidebarManager.init(true);
        }, 500);
      }
